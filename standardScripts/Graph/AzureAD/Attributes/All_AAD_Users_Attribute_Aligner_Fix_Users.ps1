...
{
    If ($user.CompanyName -eq "uniqueParentCompany Alcoil Inc")
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-2" 
    }
    ...
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-2" 
     ...
     If ($user.CompanyName -eq "uniqueParentCompany-Alcoil")
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-2" 
    }
    ...
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-20" 
    }
    ...
    If ($user.CompanyName -eq "anonSubsidiary-1")
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-20" 
    }

    If ($user.CompanyName -eq "uniqueParentCompany Select")
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-15" 
    }


    If ($user.CompanyName -eq "unique-Office-Location-21")
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-15" 
    }
    ...
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-21" 
    }
    ...
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-21" 
     ...
     If ($user.CompanyName -eq "uniqueParentCompany Dry Cooling, Inc")
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-5" 
    }

     If ($user.CompanyName -eq "anonSubsidiary-1, Inc")
    {
    Write-Host "$($user.DisplayName) company name is being updated"
    Update-MGUser -UserId $user.UserPrincipalName -CompanyName "unique-Company-Name-18" 
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCxZAxyUAr4kFjF
# us8NjUHH22hTVqPndgfmlwXPtyXPwKCCFAUwggWQMIIDeKADAgECAhAFmxtXno4h
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIAQzncSzOAQyMUBHbmy6SLyx
# WUygGjBzGqERGIf8NL6sMA0GCSqGSIb3DQEBAQUABIICACntmi46IAzPifjEMhbU
# XXszWSI9NQepElA2e1HZ89Nof1p2mqQ0YA92VXGGyc0EKMeOE3bQX2W3mk07/PHx
# kbVgEoUrIQLGlnfCZOIpVqXNzedJB2xTH7ts/iaxpnadankpGLB9ubUuxV9Xn1Lo
# BnGf+nBBVZvHqtc1Mvl5RrMaosGvyqW3zG26g+VTJDNUTGQxjVU4/9+Ih00OaHUu
# hi5or1si0AH9wZ8h73cBj1X9dMxj0aa7O6CJsNmopVVyPSEDdNg0nNtYcirFMFOv
