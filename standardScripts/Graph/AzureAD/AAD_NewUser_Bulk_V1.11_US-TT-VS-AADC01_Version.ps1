...
$pendingRequests = Invoke-RestMethod -Method get -uri 'https://uniqueParentCompany.atlassteamMember.net/rest/api/2/search?jql=summary%20~%20"Onboard%20Request"' -Headers $headers
...
foreach ($ticket in $pendingRequests.issues)
    {
        ...
        Else
        {
            ...
            $emailAddr = $firstNameUPN + "." +$lastNameUPN + $uData.customfield_10766
...
If (Get-MGUser -UserId $emailaddr -erroraction SilentlyContinue)
{
   ...
   #Generate a new UPN
   $emailAddr = $firstNameUPN + "."+$middleInitial+"."+$lastNameUPN + $uData.customfield_10766
   ...
   If (Get-MGUser -UserId $emailaddr -erroraction SilentlyContinue)
    {
        ...
        $emailAddr = $firstNameUPN + "."+$middleInitial+"."+$lastNameUPN +$nameSuffix+ $uData.customfield_10766
...
"@ 
            ...
            #Licenses Detection - If out of licenses, notate and move to next entry
            $sku1 = Get-MgSubscribedSku -All | Where SkuPartNumber -eq $uData.customfield_10774
	
	        $sku1.prepaidunits.enabled - $sku1.consumedunits 
...
$jsonPayload = @"
    ...
    "update": {
            "comment": [
                {
                    "add": {
                        "body": "Automation failed, $($uData.customfield_10774) licenses need purchased"
                    }
                }
            ]
        },
...
$jsonPayload = @"
    ...
    "update": {
            "comment": [
                {
                    "add": {
                        "body": "Automation failed, $($uData.customfield_10775) licenses need purchased"
                    }
                }
            ]
        },
...
"@ 
            ...
            Else
            {
                ...
                if ($udata.customfield_10777 -in "IT","CA","BE","AU","DE","DK","VN","AE","MY","GB","ZA")
                {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDK2ClY+PNTZSDq
# ppXCtFe+Sr+iSx4JFo2lYLUG/Oi766CCFAUwggWQMIIDeKADAgECAhAFmxtXno4h
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIAOvUjwpardotLFyyYAUgiby
# Y+rPsz16gV8juLQ4vGRbMA0GCSqGSIb3DQEBAQUABIICABFYB6v0TVXrc9vThSrz
# TLpQI1wKNWAawSOFbPyhEpjTaMV52IqpJiNKgVAipaGe84FclhN8l0XgW+GvjU2/
# gPMTTAxxWydLYfUIKfQieKFgcBe+OPtW4gN8vEld0qods/jFOKx4+pt1h2IQANI+
# fXmfYHk8eue0/R9cpepaSyn4obFwVotHwDYqr67IZS2IzEOpqtnsZKFlLlpxzdKA
# MxTpPckR7UQqUz97VGP8OGBWfxI4Vx5HWKE/d660K/QYelAlH5OoKx3aPGhpnqhP
# CtjSOqUpwCtm1nlJC4YLO4e9htroGxKLCpfHF/XOFdyM+G21XjWuq/rRr+STWblY
# xEAlNs49f1YheBpVM/zus5BrVF4eEtIGVhlYu0ZhwolmOHlCcPtzt8WK2ARPEjN7
